...
use DirHandle;
use strict;
use vars qw($Is_Mac $Is_OS2 $Is_VMS $Is_Win32 $Is_Dos $Is_PERL_OBJECT 
            $Verbose %pm %static $Xsubpp_Version  $VERSION);
...
sub curdir {
    # Why not hardcode this?  We're paving the way to slice some of
    # MM_Unix out into MM_Generic (or somesuch)
...
sub rootdir {
    # Why not hardcode this?  See above.
...
###    $self->{ROOTEXT} = ($Is_VMS ? '' : '/') . $self->{ROOTEXT} if $self->{ROOTEXT};
	...
	my($dir);
	foreach $dir ($self->updir(),
                      $self->catdir(($self->updir()) x 2),
                      $self->catdir(($self->updir()) x 3),
                      $self->catdir(($self->updir()) x 4),
                     )
        {
	    if (
...
$(PERL_ARCHLIB)/Config.pm: $(PERL_SRC)/config.sh
    ...
    if( $self->{OBJECT} ) {
        my @headers = glob "$self->{PERL_INC}/*.h";
        my $make_hdrs = "\nPERL_HDRS = \\\n    ";
        $make_hdrs .= join " \\\n    ", 
          map { s|^.*/||; '$(PERL_INC)/'.$_ } @headers;
